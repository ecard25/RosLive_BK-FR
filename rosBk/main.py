...
from database import init_db, get_db, UsuarioModel, PeticionModel, SalaModel
...
async def lifespan(app: FastAPI):
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.commit()
            print("Columna nombre_real verificada/añadida con éxito.")
    except Exception as e:
        print(f"Error al verificar/añadir columna nombre_real: {e}")
    yield
...
async def crear_sala(
    ...
    async with httpx.AsyncClient() as client:
        # Lógica: Si SALA_ID_UNICA es None, creamos sala. Si no, usamos la que existe.
        if SALA_ID_UNICA is None:
            ...
            meeting_response = await client.post(meetings_url, json={"title": sala_data.title or "Clase"}, headers=headers)
            meeting_data = meeting_response.json()
            SALA_ID_UNICA = meeting_data["data"]["id"]
            print(f"--- Sala nueva creada: {SALA_ID_UNICA} ---")
            ...
            print(f"--- Reutilizando sala existente: {SALA_ID_UNICA} ---")
        
        meeting_id = SALA_ID_UNICA
        
        # Agregar participante con restricciones de solo audio
